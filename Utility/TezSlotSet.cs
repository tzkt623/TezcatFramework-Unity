...
namespace tezcat.Utility
{
    public class TezSlot
    {
        ...
        /// 槽物品
        /// </summary>
        public ITezItem item { get; set; }

        /// <summary>
        /// 下一个分割槽
        /// </summary>
        public TezSlot next_split { get; set; } = null;
    ...
    public abstract class TezSlotSet<T> where T : TezSlot, new()
    {
        ...
        protected T createSlot()
        {
            if (m_EmptySlots.Count > 0)
            {
                return m_EmptySlots.Dequeue();
            }
            else
            {
                var slot = new T();
        ...
        public void add(TezEventBus.Action<T> set_slot)
        {
            var slot = this.createSlot();
            set_slot(slot);
        }

        public void set(int slot_id, TezEventBus.Action<T> set_slot)
        {
