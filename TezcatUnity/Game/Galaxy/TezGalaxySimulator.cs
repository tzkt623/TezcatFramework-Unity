...
namespace tezcat.Framework.Game
{
    public class TezGalaxySimulator
    {
            ...
            int star_count)
        {
            ...
            {
                var radius = m_CDF.valFromProb(UnityEngine.Random.value);
                ...
                star.theta = UnityEngine.Random.value * 360.0f;
                star.velocityTheta = this.calculateOrbitalVelocity((star.offsetWidth + star.offsetHeight) / 2.0f);
                star.center = new Vector3(0, 0, ((UnityEngine.Random.value * 2) - 1) * 40f);
                star.temperature = 6000 + (4000 * UnityEngine.Random.value) - 2000;
                star.brigtness = 0.3f + 0.2f * UnityEngine.Random.value;
                m_Stars[i] = star;
            }
            #endregion
            ...
            for (int i = 0; i < this.dustCount; i++)
            {
                if (i % 4 == 0)
                {
                    rad = m_CDF.valFromProb(UnityEngine.Random.value);
                }
                else
                {
                    x = 2 * this.radiusGalaxy * UnityEngine.Random.value - this.radiusGalaxy;
                    y = 2 * this.radiusGalaxy * UnityEngine.Random.value - this.radiusGalaxy;
                ...
                dust.theta = 360.0f * UnityEngine.Random.value;
                dust.velocityTheta = this.calculateOrbitalVelocity((dust.offsetWidth + dust.offsetHeight) / 2.0f);
                dust.center = new Vector3(0, 0, ((UnityEngine.Random.value * 2) - 1) * 40f);
                dust.temperature = 5000 + rad / 4.5f;
                dust.brigtness = 0.015f + 0.01f * UnityEngine.Random.value;
                m_Dust[i] = dust;
            ...
            for (int i = 0; i < h2Count; ++i)
            {
                ...
                h2_1.theta = 360.0f * UnityEngine.Random.value;
                h2_1.velocityTheta = this.calculateOrbitalVelocity((h2_1.offsetWidth + h2_1.offsetHeight) / 2.0f);
                h2_1.center = new Vector3(0, 0, ((UnityEngine.Random.value * 2) - 1) * 40f);
                h2_1.temperature = 6000 + (6000 * UnityEngine.Random.value) - 3000;
                h2_1.brigtness = 0.1f + 0.05f * UnityEngine.Random.value;
                m_H2[k1] = h2_1;
